...
&ensp;&ensp;&ensp;&ensp;连贯性是文本质量评估的一个重要方面，也是保证其可读性的关键。现有连贯性模型的一个重要限制是，在一个域上进行训练不会轻易地推广到其他领域的文本类别。先前的工作主张跨域泛化的生成模型，因为判别模型，训练过程中要区分的不连贯句子顺序的空间过大。在这项工作中，我们提出了一个具有较小负采样空间的局部判别神经模型，该模型可以有效地学习错误的排序，且该模型结构简单，并在华尔街的标准基准数据集上明显优于以前的最新方法期刊语料库，以及在转移到Wikipedia文章上看不见的论述类别的多个新的挑战性环境中。
